    ...
    public bool Execute()
    {
        ...
        FluentConfigUi.Create(CPH, Title, Version)
            ...
            .Section("Voices", "Voices", s => s
                ...
                .Dropdown("Default voice alias", "default_voice_alias")
                    .Hint("Voice for users who haven't purchased. Click Refresh to update from current aliases (no save needed).")
                    .Options(GetVoiceAliasesFromSettings())
                    .Refresh(ctx => ctx.GetPendingValue<string[]>("tts_voice_aliases") ?? Array.Empty<string>())
                    .DefaultIndex(0)
            )
