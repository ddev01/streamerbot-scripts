...
internal static class FluentConfigLaravelFormatter
{
    ...
    private sealed class Rewriter : CSharpSyntaxRewriter
    {
        ...
        {
            // Nested s => s.... bodies are handled in VisitSimpleLambdaExpression.
            if (node.Parent is SimpleLambdaExpressionSyntax lambda && lambda.ExpressionBody == node)
        ...
        public override SyntaxNode? VisitSimpleLambdaExpression(SimpleLambdaExpressionSyntax node)
        {
            ...
            if (rewritten.ExpressionBody is not InvocationExpressionSyntax body)
                return rewritten;

            if (!TryFlatten(body, out var rootExpr, out var links))
                return rewritten;

            if (!IsIdentifierRoot(rootExpr) || !IsFluentConfigChain(rootExpr, links))
                return rewritten;

            var stmtIndent = GetStatementIndent(rewritten);
            var formattedBody = FormatChain(rootExpr, links, stmtIndent, isLambdaParamRoot: true);
            var newBody = SyntaxFactory.ParseExpression(formattedBody);
            return rewritten.WithExpressionBody(newBody);
        }
    }
    ...
    private static SyntaxNode ParseExpr(string formatted, SyntaxNode triviaSource)
    {
        ...
        return expr.WithLeadingTrivia(triviaSource.GetLeadingTrivia())
            .WithTrailingTrivia(triviaSource.GetTrailingTrivia());
    }

    private static bool IsChainRoot(InvocationExpressionSyntax node)
    {
        return node.Parent is not MemberAccessExpressionSyntax ma
            || ma.Parent is not InvocationExpressionSyntax
            || !ReferenceEquals(ma.Expression, node);
    }

    private static bool TryFlatten(
        ExpressionSyntax node,
        out ExpressionSyntax rootExpr,
        out List<(string Name, ArgumentListSyntax Args)> links
    )
    {
        links = [];
    ...
    private static bool IsFluentConfigChain(
        ExpressionSyntax rootExpr,
        List<(string Name, ArgumentListSyntax Args)> links
    )
    {
        ...
        {
            if (
                UiMethods.Contains(name)
                || ControlMethods.Contains(name)
                || OptionMethods.Contains(name)
            )
                return true;
        }
    ...
    )
    {
        ...
        for (var i = 0; i < links.Count; i++)
        {
            ...
            sb.Append(
                FormatArgumentList(
                    args,
                    ContinuationIndent(name, stmtIndentCols, isLambdaParamRoot)
                )
            );
        }
    ...
    private static string FormatArgument(ArgumentSyntax arg, int chainIndentCols)
    {
        if (arg.Expression is SimpleLambdaExpressionSyntax { ExpressionBody: not null } lambda)
        {
            ...
            if (
                lambda.ExpressionBody is InvocationExpressionSyntax body
                && TryFlatten(body, out var rootExpr, out var links)
                && IsFluentConfigChain(rootExpr, links)
            )
            {
                var bodyText = FormatChain(
                    rootExpr,
                    links,
                    chainIndentCols,
                    isLambdaParamRoot: true
                );
    ...
    private static int GetStatementIndent(SyntaxNode node)
    {
        ...
        var target = node.AncestorsAndSelf().FirstOrDefault(a =>
            a is StatementSyntax or MemberDeclarationSyntax or EqualsValueClauseSyntax
        ) ?? node;
